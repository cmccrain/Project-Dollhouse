...
namespace TSOClient.Code.Rendering.City
{
    ...
    public class CityData
    {
        ...
        public static CityData Load(GraphicsDevice gd, string path)
        {
            ...
            Texture2D elevationTexture = Texture2D.FromStream(gd, File.Open(Path.Combine(path, "elevation.bmp"), FileMode.Open));
            Texture2D terrainTexture = Texture2D.FromStream(gd, File.Open(Path.Combine(path, "terraintype.bmp"), FileMode.Open));
            Texture2D vertexTexture = Texture2D.FromStream(gd, File.Open(Path.Combine(path, "vertexcolor.bmp"), FileMode.Open));
